...
(function () {
      ...
      <div class="product-popup-overlay" id="productPopupOverlay" role="dialog" aria-modal="true" aria-labelledby="popupProductTitle">
        <div class="product-popup-modal">
          ...
          <div class="popup-upsection">
            ...
            <div class="product-popup-options">
              <div class="product-popup-variants">
                ...
                <div class="product-popup-sizes">
                  ...
                  <div class="product-popup-custom-dropdown" id="popupSizeDropdown">
                    <button type="button" class="product-popup-dropdown-trigger" id="popupSizeTrigger">
                      Choose your size
                    </button>
                    <div class="product-popup-dropdown-options" id="popupSizeOptions"></div>
                  </div>
                </div>
              </div>
  ...
  function renderColorVariants(product) {
    ...
    let slider = colorButtonsContainer.querySelector('.product-popup-color-slider');
    if (!slider) {
      slider = document.createElement('div');
      slider.className = 'product-popup-color-slider';
      colorButtonsContainer.appendChild(slider);
    }
    // Hide slider initially (no selection)
  ...
  // ============================================
  // SELECT COLOR
  // WHY: Updates selected color and button styling with sliding animation
  // Default: Black text on transparent background
  // Active/Clicked: White text with black sliding background
  // ============================================
  function selectColor(color, buttonElement) {
    selectedColor = color;

    const colorButtonsContainer = document.getElementById('popupColorButtons');
    const slider = colorButtonsContainer.querySelector('.product-popup-color-slider');

    // Remove active class from all buttons
    const allButtons = colorButtonsContainer.querySelectorAll('.product-popup-color-btn');
    ...
    // Animate slider to the clicked button position
    if (slider) {
      ...
      // Apply transform and width for smooth sliding
  ...
  function populateSizeDropdown() {
    const optionsContainer = document.getElementById('popupSizeOptions');
    const trigger = document.getElementById('popupSizeTrigger');
    const dropdown = document.getElementById('popupSizeDropdown');
  ...
  function handleAddToCart() {
    // Validation
    if (!selectedColor || !selectedSize) {
      console.error('Product popup script not loaded');
      return;
    }
